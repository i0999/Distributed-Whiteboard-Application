...
async function readSnapshotIfExists(roomId: string) {
    const repository = await RepositoryFactory.getRoomRepository();
    let room = await repository.getRoomById(roomId);
    if (!room) {
        room = await repository.createRoom(roomId);
    }
    return room.drawObjects ?? undefined;
}

async function saveSnapshot(roomId: string, snapshot: RoomSnapshot) {
    const repository = await RepositoryFactory.getRoomRepository();
    await repository.updateRoom(roomId, snapshot);
}
