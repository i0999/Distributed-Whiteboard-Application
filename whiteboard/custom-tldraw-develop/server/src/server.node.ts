...
const PORT = Number.parseInt(process.env.VITE_APP_SERVER_PORT || '5858');
const HOST = process.env.VITE_HOST || '0.0.0.0';
const META_POD_NAME = process.env.META_POD_NAME || 'POD Name' + Math.random().toString();
const META_NODE_NAME = process.env.META_NODE_NAME || 'NODE Name' + Math.random().toString();
...
app.register(async (app) => {
    ...
    app.get('/connect/:roomId', { websocket: true }, async (socket, req) => {
        console.log('Connecting to room on', META_NODE_NAME, META_POD_NAME);
